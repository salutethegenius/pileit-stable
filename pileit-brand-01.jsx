...
const Mark = ({ size = 100, bg = "transparent" }) => {
  ...
  return (
    <svg
      ...
      style={{ display: "block", background: bg, borderRadius: bg !== "transparent" ? size * 0.18 : 0 }}
    >
      <defs>
      ...
      {/* Inner arrow fill — slightly visible so shape reads at small sizes */}
      <polygon
        points={`${play.x1},${play.y1} ${play.x2},${play.y2} ${play.x3},${play.y3}`}
        fill={ORANGE_DEEP}
        opacity={playInnerOpacity}
      />
      {/* Arrow border — thicker in user space when size ≤36px (nav) so the cutout reads */}
      <polygon
        points={`${play.x1},${play.y1} ${play.x2},${play.y2} ${play.x3},${play.y3}`}
        fill="none"
        stroke={ORANGE_LIGHT}
...
const Wordmark = ({ size = 32, color = WHITE }) => (
  <div style={{
    ...
    fontSize: size, letterSpacing: size * 0.08,
...
const Swatch = ({ bg, label, children }) => (
  <div style={{
    background: bg, borderRadius: 12, padding: "32px 28px",
    display: "flex", flexDirection: "column", gap: 20, flex: 1
  }}>
    <div style={{ color: bg === WHITE ? "#333" : "#555", fontSize: 11, letterSpacing: 2, fontFamily: "'Barlow Condensed', Impact, sans-serif" }}>
      {label}
    </div>
    {children}
  </div>
...
export default function PileItBrand() {
  ...
  return (
    <>
      ...
      <div style={{ background: BG, minHeight: "100vh", padding: "48px 40px", fontFamily: "'Barlow Condensed', Impact, sans-serif", color: WHITE }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>

          {/* Header */}
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 48 }}>
            <div>
              <div style={{ color: ORANGE, fontSize: 11, letterSpacing: 4, fontStyle: "italic", marginBottom: 6 }}>
                SIGNAL K SYSTEM · CONCEPT 01 · REFINED
              </div>
              <div style={{ fontSize: 38, fontWeight: 800, fontStyle: "italic", letterSpacing: 2 }}>
                THE STACK PLAY
              </div>
              <div style={{ color: "#666", fontSize: 15, fontStyle: "normal", fontWeight: 400, marginTop: 4 }}>
          ...
          <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #222", marginBottom: 48 }}>
            {tabs.map(t => (
              <button key={t} onClick={() => setTab(t)} style={{
                ...
                fontFamily: "'Barlow Condensed', Impact, sans-serif",
                fontSize: 14, fontStyle: "italic", fontWeight: tab === t ? 800 : 600,
                letterSpacing: 1, marginBottom: -1, textTransform: "uppercase",
                transition: "all 0.2s"
              }}>{t}</button>
            ))}
          </div>
          ...
          {tab === "mark" && (
            <div>
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: 40 }}>
                <Mark size={200} />
                <div style={{ color: "#444", fontSize: 13, lineHeight: 1.9, maxWidth: 340, fontStyle: "normal", fontWeight: 400 }}>
                  Three bold horizontal bars — a visual pile — with a play arrow cleanly cut through all three layers. The bars read as a content stack or playlist. The play shape is inseparable from the pile: streaming and stacking are a single gesture.<br /><br />
                  <span style={{ color: ORANGE }}>Gradient:</span> #FB923C → #F97316 → #C2410C<br />
                  <span style={{ color: ORANGE }}>Shine:</span> 28% white top, fades to 0<br />
          ...
          {tab === "lockup" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 40 }}>
              ...
              <div>
                ...
                <div style={{ padding: "28px 36px", background: "#111", borderRadius: 12, display: "flex", gap: 24, alignItems: "flex-end" }}>
                  <Mark size={80} />
                  <Mark size={48} />
                  <Mark size={32} />
                  <Mark size={20} />
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
                {[
                  ...
                  <div key={size} style={{ display: "flex", alignItems: "center", gap: 24 }}>
                    <div style={{ width: 180, color: "#444", fontSize: 12 }}>{label} · {size}px</div>
                    <Mark size={size} />
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 16 }}>
                ...
                <div style={{
                  ...
                  alignItems: "center", gap: 10, width: 160
                }}>
                  <AppIcon size={72} />
                  <div style={{ fontSize: 12, color: WHITE, fontStyle: "normal", fontWeight: 600 }}>PileIt</div>
                </div>
                {/* Warm home screen */}
                <div style={{
                  ...
                  <AppIcon size={72} />
                  <div style={{ fontSize: 12, color: "#ddd", fontStyle: "normal", fontWeight: 600 }}>PileIt</div>
                </div>
              </div>
            </div>
          )}
          ...
          {tab === "backgrounds" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div style={{
                ...
                justifyContent: "space-between", height: 64, borderRadius: 10
              }}>
                <Lockup markSize={32} textSize={22} gap={10} />
                <div style={{ display: "flex", gap: 24, color: "#999", fontSize: 14, fontStyle: "normal" }}>
                  <span>Browse</span><span>Creators</span><span>Live</span>
                </div>
                <div style={{ display: "flex", gap: 10 }}>
                  <div style={{ background: "#222", borderRadius: 6, padding: "7px 16px", fontSize: 13, color: "#999" }}>Log in</div>
                  <div style={{ background: ORANGE, borderRadius: 6, padding: "7px 16px", fontSize: 13, color: WHITE, fontWeight: 700 }}>Sign up</div>
                </div>
              </div>
            </div>
          )}
